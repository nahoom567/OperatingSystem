...
inline void setNewPageEntry(PageTable* currentPageTable, uint64_t entryIndex, bool pageTableIndex, uint64_t physicalPageIndex)
{
    ...
    if(pageTableIndex) { pageAddress                = requestIdentityMappedPage(); }
...
inline uint64_t createOrFindNextPageIndex(uint64_t pageIndex, uint64_t entryIndex, bool pageTableIndex = true, uint64_t physicalPageIndex = NULL, bool overrunTableEntry = false)
{
    ...
    {
        setNewPageEntry(pageTable, entryIndex, pageTableIndex, physicalPageIndex);
    }
    ...
    else if (!pageTableIndex && overrunTableEntry)
    {
        setNewPageEntry(pageTable, entryIndex, pageTableIndex, physicalPageIndex);
    }
...
bool mapMemory(PhysicalAddress pAddr, VirtualAddress vAddr, bool overrunTableEntry)
{
    ...
    for(uint64_t i = 0; i < PAGE_TABLE_MAX_LEVEL; i++)
    {
        ...
        pageIndex = createOrFindNextPageIndex(pageIndex, pageTableEntriesIndexes[i], isPageTableIndex, pAddr.addr.pageIndex, overrunTableEntry);
    }
