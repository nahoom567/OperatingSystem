...
{
    // we start from THE_AMOUNT_PAGES_THAT_HAVE_IDENTITY_MAPPING + 1because we would like to reserve pages with identity mapping
    // to page tables and kernel mode memory
    return getPageWithInRange(THE_AMOUNT_PAGES_THAT_HAVE_IDENTITY_MAPPING + 1, getPageCount());
}
...
(
    void printTakenPages()
    {
        ...
        {
            // printing the index if the bit cannot be used: 
            if((*pageBitMap)[i] > 0)
            {
                printInt(i);
                printChar(',');
